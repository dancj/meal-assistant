...
- `src/components/` — UI components: `home-page.tsx` (the client page body, accepts `emailEnabled`), `deals-sidebar.tsx`, `grocery-list.tsx`, `email-button.tsx` (#70), the Editorial Week-screen domain components (`meal-row.tsx`, `day-label.tsx`, `theme-pill.tsx`, `cadence-pulse.tsx`, `kid-note.tsx`) shipped in Phase 2 of the UI makeover (#86), and the SwapDrawer follow-up (`swap-drawer.tsx`, `swap-suggestion.tsx`) that replaces the old Swap-button-as-regenerate flow with a 420px right-side drawer of three ranked, swap-in-place suggestions. Editorial primitives (`pill.tsx`, `eyebrow.tsx`, `hairline-list.tsx`, `drawer.tsx`, refreshed `button.tsx` and `dialog.tsx`/`Modal` aliases) live under `src/components/ui/` alongside the remaining shadcn primitives.
- `src/lib/` — shared utilities. Currently: `api/` (typed client wrappers `fetchRecipes`, `fetchDeals`, `fetchRecentLogs`, `fetchPantry`, `postMealLog`, `generatePlan`, `sendEmail` + `ApiError`), `recipes/` (GitHub-backed recipe reader), `deals/` (Flipp-backed Safeway + Aldi deals), `plan/` (Claude-powered meal-plan generator + `assertMealPlan` shape helper), `log/` (multi-doc YAML log parser + GitHub upsert + recent-weeks reader), `pantry/` (frontmatter parser + single-file GitHub reader for `/pantry.md`), `plan-ui/` (client-side state machine + `usePlanState` React hook + thumb/skip-reason/swap-drawer wiring; `week.ts` `currentWeekStart` ISO-string helper for the email/log routes, delegating to `week-ui` for the underlying Date math), `week-ui/` (Phase-2 client-side synthesis shim — date math, ISO 8601 week numbering, theme keyword matching, per-day metadata derivation; replace when `/api/generate-plan` ships richer per-meal metadata), `swap-ui/` (Phase-2 client-side ranker + suggestion synthesis for the SwapDrawer — protein keyword extraction, lastMade lookup against `MealLog`, theme/protein-rotation scoring; replace when `/api/generate-plan` ships per-slot swap suggestions), `email/` (HTML+text formatter + Resend sender + typed errors for `/api/email`), `demo/fixtures.ts` (sample recipes/deals/plan/logs/pantry returned when `DEMO_MODE=1`), `resend.ts` (lazy Resend client factory), `email.ts` (`parseRecipients` helper), `utils.ts` (`cn` className merger)
